...
import { type GetPoolResult, Pool } from '@/services/PoolService';
...
const poolDetail = ref<GetPoolResult>();

const getPoolDetail = async () => {
  const res = await Pool.GetPool({
    name: poolName.value,
  });

  poolDetail.value = res;
};
